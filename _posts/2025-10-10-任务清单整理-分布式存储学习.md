...
## 任务(25.10.28)
* 任务1：补充`conductor`命令
* 任务2(已完成待检视,在143环境上): 修改pfstore增加` delete existing trays & shard_disk` 策略 - 详见"分析t_tray表格冗余问题"
参考[https://github.com/cocalele/PureFlash/pull/102](https://github.com/cocalele/PureFlash/pull/102)
* 任务3: 修改`v_store_alloc_size`逻辑，增加实际已分配的空间，解决总空间比分配空间小的问题


## 任务conductor命令开发(25.10.31)

目前进展：完成open_volume/open_aof, update_volume, list_port(限制连接本机，即-i 1)
暂无法完成：sanity_check(s5bd等进程不存在), expose_volume(缺少s5bd)
